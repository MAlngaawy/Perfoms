...
const HealthSteps = (props: Props) => {
  ...
  const [data, setData] = React.useState({
    ...
    steps: 0,
    info: [
      { name: "Duration", label: "0 h", imgSrc: "/assets/images/time.png" },
      {
        name: "Total Distance",
        label: "0 km",
  ...
  const calculateSteps = React.useCallback(() => {
    ...
    const startTime = [
      ...DataSteps?.map(
        (bucket: { dataset: [point: [{ startTimeNanos: string }]] }) =>
          bucket?.dataset?.map((pointItem: any) =>
            pointItem?.point?.map(
              (pointValue: any) => pointValue.startTimeNanos
            )
          )
      ),
    ...
    const endTime = [
      ...DataSteps?.map(
        (bucket: { dataset: [point: [{ endTimeNanos: string }]] }) =>
          bucket?.dataset?.map((pointItem: any) =>
            pointItem?.point?.map((pointValue: any) => pointValue.endTimeNanos)
          )
      ),
    ]?.reduce((a: string, b: string): number => Number(a) + Number(b), 0);
    ...
    return {
      ...
      info: [
        {
          name: "Duration",
          label: `${Time?.toFixed(1) || 0}  h`,
          imgSrc: "/assets/images/time.png",
        },
        {
          name: "Total Distance",
          label: `${distanceKm?.toFixed(1) || 0} km`,
          imgSrc: "/assets/images/map.png",
        },
      ],
    };
  ...
  return (
    <div className="HealthSteps bg-white rounded-3xl w-full grid gap-9 p-4 h-full">
      ...
      <div className=" w-full flex justify-center ">
        <div className="relative rounded-tl-[50%] p-5 pr-6 pb-0 rounded-tr-[50%] flex justify-center h-[152px] overflow-hidden">
          <RingProgress
            ...
            sections={[
              {
                value: +data?.progressValue > 0 ? +data?.progressValue / 2 : 0,
                color: "#E19809",
              },
            ]}
            label={
          ...
          />
          <div className="absolute bottom-0 left-0 text-[#BDBDBD] text-base">
            0
          </div>
          <div className="absolute bottom-[35%] left-[4%] text-[#BDBDBD] text-base">
            1 km
          </div>
          <div className="absolute bottom-[65%] left-[15%] text-[#BDBDBD] text-base">
            2 km
          </div>
          <div className="absolute top-2 left-auto right-auto text-[#BDBDBD] text-base">
            3 km
          </div>

          <div className="absolute bottom-[65%] right-[15%] text-[#BDBDBD] text-base">
            4 km
          </div>
          <div className="absolute bottom-[35%] right-[5%] text-[#BDBDBD] text-base">
            5 km
          </div>
          <div className="absolute bottom-0 right-0 text-[#BDBDBD] text-base">
            6 km
          </div>
        </div>
      </div>
      <div className="flex gap-x-5    ">
        {data.info.map((item) => (
          <div key={item.name} className="flex gap-2 flex-1">
            <Image width={60} height={60} src={item.imgSrc} alt={item.name} />
            <div>
              <p className="text-black text-lg font-medium  ">{item.label}</p>
              <p className="text-base text-[#828282]">{item.name}</p>
            </div>
          </div>
        ))}
      </div>
