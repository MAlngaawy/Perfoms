import { Checkbox, Grid, Menu, Input, Textarea, LoadingOverlay } from "@mantine/core";
...
more details more details more details more details more details .` },
  ...
  return (
    <div className="Help-Center grid gap-10 p-5 mb-20">
      ...
      <Grid columns={12} grow gutter={"xl"} className=" p-4 relative bg-white rounded-[10px] md:divide-x divide-[#BDBDBD] ">
        ...
        <Grid.Col span={12} md={6} className="relative grid gap-3">
          ...
          <div className="relative  flex items-stretch" >
            ...
            placeholder="Your Question" minRows={4} autosize
            onChange={(e) => setChat(e.currentTarget.value as string)}
            label={
              <button type="submit" disabled={isLoading} onKeyUp={(e) => {
                if (e.key === "Enter") { useChat({ message: chat }) }
              }} onClick={() => { useChat({ message: chat }) }} className=" !z-[50000000000] shadow min-w-[80px]  bg-white text-perfGray1 p-2 px-3 rounded-md flex justify-between items-center" >
        ...
        <Grid.Col span={12} md={6} className="relative ">
          {data?.content ? (<p className="h-full w-full whitespace-pre-wrap bg-white border rounded-[10px] p-5" ref={refContent}>
            {data?.content}
          </p>) : (<div className="h-full w-full bg-white border rounded-[10px] p-5">
            Answer
          </div>)}
        </Grid.Col>
      </Grid>
      ...
      <Grid columns={12} grow gutter={"xl"} className=" gap-10 ">
        {PopularQuestions.map(ques => (<Grid.Col span={12} key={ques.id} sm={6} md={4} lg={3} xl={2} className="relative bg-white rounded-[10px] border border-[#E1E2E9]  p-5 grid gap-4">
      ...
      <div className="p-2 bg-white rounded-[10px] flex flex-col sm:flex-row gap-3">
      <div className="  flex md:hidden gap-4  flex-wrap flex-row  p-2 justify-end">
                {value.length > 0 && (value.map(item => (
                  <div className=" bg-[#EBF3FE] flex justify-between gap-2 items-center p-2 h-[32px] rounded-2xl text-perfBlue "> {item}
                    <XCircleIcon className="w-5 h-5 text-perfGray3 " onClick={() => setValue(prev => prev.filter(val => val !== item))} />
        ...
        <div className="border border-[#BDBDBD] rounded-[10px] flex flex-1 p-3 divide-x divide-[#BDBDBD] ">
          ...
          <Input
            ...
            classNames={{ input: 'border-0  ', wrapper: " w-full flex-1", rightSection: 'md:block md:w-1/2 hidden  ' }}
            rightSection={
              <div className=" flex gap-4  flex-wrap flex-row  p-2 justify-end">
                {value.length > 0 && (value.map(item => (
                  <div className=" bg-[#EBF3FE] flex justify-between gap-2 items-center p-2 h-[32px] rounded-2xl text-perfBlue "> {item}
                    <XCircleIcon className="w-5 h-5 text-perfGray3 " onClick={() => setValue(prev => prev.filter(val => val !== item))} />
        ...
        <button type="submit" className="text-white min-w-[11rem] min-h-[2rem] h-full  bg-perfBlue shadow rounded-xl ">Search </button>
      </div>

      <h2 className="text-perfGray3 text-sm  space-x-3">results <span className="font-bold text-perfGray1 ">42</span> </h2>
      <div className="grid gap-5  ">
        {resultsData.map(item => (<div key={item.id} className="bg-white p-4 rounded-[10px] grid gap-2 ">
          <div className="flex justify-between "><h3 className="flex-1 text-2xl text-perfGray1 ">{item.title}</h3><Link to={item.link} className="text-end text-perfBlue">more details</Link></div>
